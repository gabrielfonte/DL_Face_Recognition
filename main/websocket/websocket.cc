...
void websocketTask(void *pvParameters){
	for(;;){
		...
		if(xQueueReceive(xWebsocketQueue, &ws_data, portTICK_PERIOD_MS)){
			...
			if(validation == true){
				if(xQueueSend(xRegisterQueue, &face_info, portMAX_DELAY)){
			...
			if((data->data_len+data->payload_offset) <= data->payload_len){
				...
				if(img_buffer != NULL){
					...
					if((data->data_len+data->payload_offset) == data->payload_len){
						...
						xQueueSend(xWebsocketQueue, data, portMAX_DELAY);
						img_buffer = NULL;
					}
				}
				else{
				...
				if(xQueueSend(xWebsocketQueue, data, portMAX_DELAY) != pdTRUE){
...
void websocket_client_start(){
	...
	xWebsocketQueue = xQueueCreate(1, sizeof(esp_websocket_event_data_t));
	xRegisterQueue = xQueueRegister;
	xQueueEvaluation = xQueueEvaluate;
	xQueueWebsocketInput = xQueueWebsocket;
	xQueueWebsocketOutput = xWebsocketResponseQueue;
