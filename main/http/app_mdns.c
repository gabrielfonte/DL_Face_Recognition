...
static char* generate_hostname(void)
{
    ...
    return strdup(CONFIG_ESP_HOST_NAME);
#else
    uint8_t mac[6];
    char   *hostname;
    esp_read_mac(mac, ESP_MAC_WIFI_STA);
    if (-1 == asprintf(&hostname, "%s-%02X%02X%02X", CONFIG_ESP_HOST_NAME, mac[3], mac[4], mac[5])) {
        abort();
    }
    return hostname;
#endif
...
void app_mdns_main(){
    initialise_mdns();
    xTaskCreate(mdns_example_task, "mdns-task", 2048, NULL, 2, NULL);
}